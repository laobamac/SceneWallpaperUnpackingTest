...
class ParticleBuilder {
        ...
        if let sub = parseParticleObject(
            ...
            if let over = overrideData {
                ...
                if let cnStr = over.colorn?.value {
                    let parts = cnStr.components(separatedBy: " ").compactMap {
                        Float($0)
                    }
                    if parts.count >= 3 {
    ...
    private static func parseParticleObject(
        ...
        if let sm = json["sequencemultiplier"] as? NSNumber {
            sub.sequenceMultiplier = sm.floatValue
            Logger.log(
                "[Particle] Sequence Multiplier: \(sub.sequenceMultiplier)"
            )
        }
        if let matPath = json["material"] as? String {
            sub.material.fileName = matPath
            do {
                let matURL = baseFolder.appendingPathComponent(
                    "materials/\(matPath)"
                )
                let finalMatURL =
                    FileManager.default.fileExists(atPath: matURL.path)
                    ? matURL
                    : baseFolder.appendingPathComponent("assets/\(matPath)")
                if FileManager.default.fileExists(atPath: finalMatURL.path) {
                    let data = try Data(contentsOf: finalMatURL)
                    if let matJSON = try JSONSerialization.jsonObject(
                        with: data
                    ) as? [String: Any] {
                        if let passes = matJSON["passes"] as? [[String: Any]],
                ...
                Logger.error("[Particle] Material parse error: \(matPath)")
            }
        }
        if let instanceOverride = parentJSON?["instanceoverride"]
            as? [String: Any]
        {
        ...
        for em in emittersJSON {
            if let op = parseEmitter(json: em) { sub.addEmitter(op) }
        }
        ...
        if let ops = json["operator"] as? [[String: Any]] {
            for op in ops {
                if let oper = parseOperator(json: op) { sub.addOperator(oper) }
            }
        }
        ...
        if let cps = json["controlpoint"] as? [[String: Any]] {
            for cp in cps {
                ...
                if let off = cp["offset"] as? String {
                    pcp.offset = parseVec3(off)
                }
                if let flags = cp["flags"] as? Int {
                    if (flags & 1) != 0 { pcp.linkMouse = true }
                    if (flags & 2) != 0 { pcp.worldSpace = true }
                }
                if pcp.id >= 0 && pcp.id < 8 { sub.controlPoints[pcp.id] = pcp }
            }
        }
        return sub
    }
        ...
        -> ParticleSubSystem?
    {
        ...
        let type = ParticleSpawnType(rawValue: typeStr) ?? .static
        ...
        do {
            ...
            guard
                let childParticleJSON = try JSONSerialization.jsonObject(
                    with: data
                ) as? [String: Any]
            else { return nil }
            if let childSub = parseParticleObject(
                ...
                if let mc = json["maxcount"] as? Int {
                    childSub.maxCountInstance = mc
                }
                if let prob = json["probability"] as? Float {
                    childSub.probability = prob
                }
                Logger.log(
                    "[Particle] Loaded child: \(name) [Type: \(typeStr)]"
                )
                return childSub
            }
        } catch { Logger.error("[Particle] Child load failed: \(name)") }
        return nil
    }
    ...
    private static func parseEmitter(json: [String: Any]) -> ParticleEmittOp? {
        ...
        if name == "boxrandom" {
            ...
            let sphere = ParticleSphereEmitter(
                directions: dirs,
                minDistance: distMin.x,
                maxDistance: distMax.x,
                emitSpeed: rate,
                origin: origin,
                sign: SIMD3<Int32>(0, 0, 0),
                onePerFrame: false,
                sort: false,
                instantaneous: instant,
                minSpeed: speedMin,
    ...
    private static func parseInitializer(json: [String: Any]) -> ParticleInitOp?
    {
        ...
        if name == "colorrandom" {
            ...
            let max = parseVec3(
                json["max"],
                defaultVal: SIMD3<Float>(255, 255, 255)
            )
            ...
            let min = parseVec3(
                json["min"],
                defaultVal: SIMD3<Float>(-32, -32, -32)
            )
            let max = parseVec3(
                json["max"],
                defaultVal: SIMD3<Float>(32, 32, 32)
            ...
            let ts = (json["timescale"] as? Double) ?? 1
            let off = (json["offset"] as? Float) ?? 0
            let smin = (json["speedmin"] as? Float) ?? 100
            let smax = (json["speedmax"] as? Float) ?? 250
            let pmin = (json["phasemin"] as? Float) ?? 0
            let pmax = (json["phasemax"] as? Float) ?? 0.1
            let fwd = parseVec3(
                json["forward"],
                defaultVal: SIMD3<Float>(0, 1, 0)
            )
            let rgt = parseVec3(
                json["right"],
                defaultVal: SIMD3<Float>(0, 0, 1)
            )
            return ParticleModules.turbulentVelocityRandom(
                scale: scale,
                timeScale: ts,
                offset: off,
                speedMin: smin,
    ...
    private static func parseOperator(json: [String: Any])
        -> ParticleOperatorOp?
    {
        guard let name = json["name"] as? String else { return nil }
        if name == "movement" {
            ...
            return ParticleModules.sizeChange(
                start: st,
                end: et,
                valStart: sv,
                valEnd: ev,
            ...
            return ParticleModules.colorChange(
                start: st,
                end: et,
                valStart: sv,
                valEnd: ev
            ...
            let pmin = (json["phasemin"] as? Float) ?? 0
            let pmax = (json["phasemax"] as? Float) ?? .pi * 2
            return ParticleModules.oscillateAlpha(
                freqMin: fmin,
                freqMax: fmax,
                scaleMin: smin,
                scaleMax: smax,
            ...
            let mask = parseVec3Int(
                json["mask"],
                defaultVal: SIMD3<Int32>(1, 1, 0)
            )
            return ParticleModules.turbulence(
                phaseMin: pmin,
                phaseMax: pmax,
                speedMin: smin,
                speedMax: smax,
    ...
    private static func parseInstanceOverride(json: [String: Any])
    ...
    private static func parseVec3(
        _ json: Any?,
        defaultVal: SIMD3<Float> = .zero
    ) -> SIMD3<Float> {
        if let str = json as? String {
            let parts = str.components(separatedBy: " ").compactMap {
                Float($0)
            }
            if parts.count >= 3 {
            ...
            let f = num.floatValue
            return SIMD3<Float>(f, f, f)
        }
        return defaultVal
    }

    private static func parseVec3Int(
        _ json: Any?,
        defaultVal: SIMD3<Int32> = .zero
    ) -> SIMD3<Int32> {
        if let str = json as? String {
            let parts = str.components(separatedBy: " ").compactMap {
                Int32($0)
            ...
            if parts.count >= 3 {
                return SIMD3<Int32>(parts[0], parts[1], parts[2])
            }
        } else if let arr = json as? [NSNumber], arr.count >= 3 {
            return SIMD3<Int32>(
                arr[0].int32Value,
