...
class ParticleSubSystem {
    ...
    var controlPoints: [ParticleControlPoint] = Array(repeating: ParticleControlPoint(), count: 8)
    ...
    init(system: ParticleSystem, maxCount: Int, rate: Float, maxCountInstance: Int, probability: Float, spawnType: ParticleSpawnType) {
        self.system = system
    ...
    func addInitializer(_ op: @escaping ParticleInitOp) { initializers.append(op) }
    func addOperator(_ op: @escaping ParticleOperatorOp) { operators.append(op) }
    ...
    func emitt(frameTime: Double) {
        ...
        for inst in instances {
            ...
            let typeHasDeath = (spawnType == .eventSpawn || spawnType == .eventFollow)
            
            if let parent = bounded.parent {
                if bounded.particleIdx != -1 && bounded.particleIdx < parent.particles.count {
            ...
            inst.particles.withUnsafeMutableBufferPointer { buffer in
                let info = ParticleInfo(particles: buffer, controlPoints: controlPoints, time: time, timePass: particleTime)
                
                for i in 0..<buffer.count {
                    if buffer[i].markNew {
                        spawnChildren(parentInst: inst, idx: i, typeCheck: { $0 == .eventFollow || $0 == .eventSpawn })
                    }
                    ...
                    if buffer[i].lifetime <= 0 {
                        spawnChildren(parentInst: inst, idx: i, typeCheck: { $0 == .eventDeath })
                    } else {
                        hasLive = true
                    }
                }
    ...
    func spawnChildren(parentInst: ParticleInstance, idx: Int, typeCheck: (ParticleSpawnType) -> Bool) {
        for child in children {
