    ...
    func makeOp() -> ParticleEmittOp {
        ...
        return { particles, inits, maxCount, timePass in
            ...
            let genBox: () -> Particle = {
                ...
                for i in 0..<3 {
                    pos[i] = ParticleMath.lerp(minDistance[i], maxDistance[i], ParticleMath.randomFloat())
                }
                var p = Particle()
                let finalPos = pos * directions
                p.position = finalPos
                let speed = ParticleMath.randomFloat(min: minSpeed, max: maxSpeed)
            ...
            if instantaneous > 0 && particles.isEmpty { emitNum = instantaneous }
            
            ParticleEmitterUtils.emitt(particles: &particles, num: emitNum, maxCount: maxCount, sort: sort) {
                return ParticleEmitterUtils.spawn(gen: genBox, inits: inits, duration: 1.0 / Double(emitSpeed))
            }
        }
    }
}
    ...
    func makeOp() -> ParticleEmittOp {
        ...
        return { particles, inits, maxCount, timePass in
            ...
            let genSphere: () -> Particle = {
                var p = Particle()
                let r = ParticleMath.lerp(minDistance, maxDistance, pow(ParticleMath.randomFloat(), 1.0/3.0))
            ...
            ParticleEmitterUtils.emitt(particles: &particles, num: emitNum, maxCount: maxCount, sort: sort) {
                return ParticleEmitterUtils.spawn(gen: genSphere, inits: inits, duration: 1.0 / Double(emitSpeed))
            }
        }
    }
}
...
struct ParticleEmitterUtils {
    static func emitt(particles: inout [Particle], num: Int, maxCount: Int, sort: Bool, spawn: () -> Particle) {
        var lastParticle = 0
        var hasDead = true
        var count = 0
        ...
        for _ in 0..<num {
            if hasDead {
                let (idx, dead) = findLastParticle(particles: particles, last: lastParticle)
                lastParticle = idx
                hasDead = dead
            }
    ...
    static func findLastParticle(particles: [Particle], last: Int) -> (Int, Bool) {
    ...
    static func spawn(gen: () -> Particle, inits: [ParticleInitOp], duration: Double) -> Particle {
        var p = gen()
        for op in inits {
            op(&p, duration)
        }
