...
fragment float4 fragment_main(VertexOut in [[stage_in]],
    ...
    struct VertexInMDL {
        float3 position [[attribute(0)]];
        uint4 blendIndices [[attribute(1)]];
        float4 blendWeights [[attribute(2)]];
    ...
    // 匹配 Swift 端的 bufferData 结构
    struct MDLObjectUniforms {
        float4x4 modelMatrix;
        float4x4 bones[128];
    };

    vertex VertexOutMDL mdl_vertex(VertexInMDL in [[stage_in]],
                                   constant GlobalUniforms &globals [[buffer(1)]],
                                   constant MDLObjectUniforms &object [[buffer(2)]])
    {
        VertexOutMDL out;
        ...
        for (int i = 0; i < 4; i++) {
            int boneIndex = in.blendIndices[i];
            float weight = in.blendWeights[i];
            if (weight > 0.0) {
                skinnedPos += (object.bones[boneIndex] * pos) * weight;
            }
        }
        ...
        // MVP: Proj * View * Model * SkinnedPos
        out.position = globals.projectionMatrix * globals.viewMatrix * object.modelMatrix * skinnedPos;
        out.texCoord = in.texCoord;
        return out;
    }

    fragment float4 mdl_fragment(VertexOutMDL in [[stage_in]],
